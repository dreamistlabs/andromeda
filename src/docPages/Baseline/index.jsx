    ...
    return baselineData.sections.map((section, i) => {
      return (
        <Section name={section.name} key={"section-" + i}>
          <Heading as={"h2"}>{h.formatHeading(section.name)}</Heading>
          <Text className={"section__description"}>{section.description}</Text>
          <Panel>{this.renderExample(section.name)}</Panel>
        </Section>
      );
    });
  }
